...
struct MeshVertex {
    inline MeshVertex () {}
    inline MeshVertex (const Vec3 & _p, const Vec3 & _n) : position (_p), normal (_n) , color (Vec3(1.,0.,0.)) , u(0) , v(0) {}
    inline MeshVertex (const MeshVertex & vertex) : position (vertex.position), normal (vertex.normal) , color (vertex.color) , u(vertex.u) , v(vertex.v) {}
    ...
    Vec3 color; // Couleur du sommet (pour l'interpolation des couleurs)
    
    std::vector<MeshTriangle> listeTriangles; // Pour connaitre les triangles auxquels appartient le MeshVertex, ça servira pour le KD Tree
};
...
public:
    ...
    Material material;
    ppmLoader::ImageRGB meshTexture;
