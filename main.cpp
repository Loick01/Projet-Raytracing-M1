...
void ray_trace_from_camera() {
    ...
    unsigned int nsamples = 15;
    std::vector< Vec3 > image( w*h , Vec3(0,0,0) );
    
    scenes[selected_scene].buildKDTree(); // Construit le KD Tree de la scène (uniquement au moment où on lance le rendu)
...
int main (int argc, char ** argv) {
    ...
    scenes[2].setup_my_scene();
    scenes[3].setup_cornell_box();
    scenes[4].setup_my_mesh();
    scenes[5].setup_complete_scene();
